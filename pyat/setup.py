...
integrator_src = '../atintegrators'
integrator_build = None

for pass_method in glob.glob(os.path.join(integrator_src, '*Pass.c')):
    print(pass_method)
    x = setup(name=pass_method[:-2], ext_modules=[Extension(name=pass_method[:-2], sources=[pass_method], define_macros=[('PYAT', None)], include_dirs=[numpy.get_include(), '../atintegrators'])])
